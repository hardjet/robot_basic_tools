...
inline Eigen::Matrix3d vectorToSkew(const Eigen::Vector3d vec) {
  Eigen::Matrix3d mat;
...
template <typename T>
inline Eigen::Matrix<T, 3, 3> vectorToSkew(const Eigen::Matrix<T, 3, 1> vec) {
...
/// \param vec :input 3-dof vector
/// \return   :output skew symmetric matrix
inline Eigen::Vector3d skewToVector(const Eigen::Matrix3d mat) {
...
/// \return
template <typename T>
inline T* eigenQtoCeresQ(const T* const q_eigen) {
...
/// \return
template <typename T>
inline T* ceresQtoEigenQ(const T* const q_ceres) {
  // Ceres convention (w, x, y, z)
  // Eigen convention (x, y, z, w)
...
/// \param theta
/// \return
inline Eigen::Quaterniond orientaionOfVectors(const Eigen::Vector3d v, const double theta) {
...
/// \param v2 :end vector
/// \return   :q_12
inline Eigen::Quaterniond orientaionOfVectors(const Eigen::Vector3d v1, const Eigen::Vector3d v2) {
...
inline Eigen::Matrix3d eularToDCM(double yaw, double pitch, double roll) {
