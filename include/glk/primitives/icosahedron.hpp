...
class Icosahedron {
 public:
  Icosahedron() {
    ...
    std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> vertices_local = {
    ...
    std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>> normals_local(vertices_local.size());
    for (int i = 0; i < vertices_local.size(); i++) {
      normals_local[i] = vertices_local[i].normalized();
    }

    std::vector<int> indices_local = {0, 11, 5,  0, 5,  1, 0, 1, 7, 0, 7,  10, 0, 10, 11, 1, 5, 9, 5, 11,
                                      4, 11, 10, 2, 10, 7, 6, 7, 1, 8, 3,  9,  4, 3,  4,  2, 3, 2, 6, 3,
                                      6, 8,  3,  8, 9,  4, 9, 5, 2, 4, 11, 6,  2, 10, 8,  6, 7, 9, 8, 1};

    this->vertices.swap(vertices_local);
    this->normals.swap(normals_local);
    this->indices.swap(indices_local);
  }
