...
CameraCalibrationTest::CameraCalibrationTest() : m_boardSize(cv::Size(0, 0)), m_squareSize(0.0f), m_verbose(false) {}
...
void CameraCalibrationTest::clear(void) {
  m_imagePoints.clear();
  m_scenePoints.clear();
...
void CameraCalibrationTest::addImage(const cv::Mat image, const std::string name) {
  cbImages.push_back(image);
  cbImageNames.push_back(name);
}

void CameraCalibrationTest::addChessboardData(const std::vector<cv::Point2f>& corners) {
...
bool CameraCalibrationTest::calibrate(void) {
...
int CameraCalibrationTest::sampleCount(void) const { return m_imagePoints.size(); }

std::vector<std::vector<cv::Point2f> >& CameraCalibrationTest::imagePoints(void) { return m_imagePoints; }

const std::vector<std::vector<cv::Point2f> >& CameraCalibrationTest::imagePoints(void) const { return m_imagePoints; }

std::vector<std::vector<cv::Point3f> >& CameraCalibrationTest::scenePoints(void) { return m_scenePoints; }

const std::vector<std::vector<cv::Point3f> >& CameraCalibrationTest::scenePoints(void) const { return m_scenePoints; }

CameraPtr& CameraCalibrationTest::camera(void) { return m_camera; }

const CameraConstPtr CameraCalibrationTest::camera(void) const { return m_camera; }

Eigen::Matrix2d& CameraCalibrationTest::measurementCovariance(void) { return m_measurementCovariance; }

const Eigen::Matrix2d& CameraCalibrationTest::measurementCovariance(void) const { return m_measurementCovariance; }

cv::Mat& CameraCalibrationTest::cameraPoses(void) { return m_cameraPoses; }

const cv::Mat& CameraCalibrationTest::cameraPoses(void) const { return m_cameraPoses; }

void CameraCalibrationTest::drawResultsInitial(std::vector<cv::Mat>& images, std::vector<std::string>& imageNames,
                                               cv::Mat& DistributedImage) const {
  drawResults(images,
              imageNames,  //
              DistributedImage, m_cameraPoseRvecsShow, m_cameraPoseTvecsShow, m_imagePointsShow, m_scenePointsShow);
}

void CameraCalibrationTest::drawResultsFiltered(std::vector<cv::Mat>& images, std::vector<std::string>& imageNames,
                                                cv::Mat& DistributedImage) const {
  drawResults(images, imageNames,
              DistributedImage,  //
              m_cameraPoseRvecsShow, m_cameraPoseTvecsShow, m_imageGoodPointsShow, m_sceneGoodPointsShow);
}

void CameraCalibrationTest::drawResults(std::vector<cv::Mat>& images, std::vector<std::string>& imageNames,
  ...
  for (i = 0; i < images.size(); ++i) {
    ...
    for (size_t j = 0; j < imagePoints.at(i).size(); ++j) {
      ...
      cv::circle(image, cv::Point(cvRound(pObs.x * drawMultiplier), cvRound(pObs.y * drawMultiplier)), r_show, green,
                 show_unit * 2, CV_AA, drawShiftBits);
      ...
      cv::circle(DistributedImage, cv::Point(cvRound(pObs.x * drawMultiplier), cvRound(pObs.y * drawMultiplier)),
                 r_show, yellow, show_unit * 2, CV_AA, drawShiftBits);
    ...
    cv::putText(image, oss.str(), cv::Point(show_unit * 10, image.rows - show_unit * 10), cv::FONT_HERSHEY_COMPLEX,
                show_unit / 2, cv::Scalar(0, 100, 255), show_unit, CV_AA);
...
void CameraCalibrationTest::writeParams(const std::string& filename) const {
  m_camera->writeParametersToYamlFile(filename);
}

bool CameraCalibrationTest::writeChessboardData(const std::string& filename) const {
...
bool CameraCalibrationTest::readChessboardData(const std::string& filename) {
...
void CameraCalibrationTest::setVerbose(bool verbose) { m_verbose = verbose; }

bool CameraCalibrationTest::calibrateHelper(CameraPtr& camera, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
...
double CameraCalibrationTest::reprojectionError(const std::vector<std::vector<cv::Point3f> >& objectPoints,
                                                const std::vector<std::vector<cv::Point2f> >& imagePoints,
...
bool CameraCalibrationTest::CalibrationDataTest(CameraPtr& camera, std::vector<cv::Mat>& rvecs,
                                                std::vector<cv::Mat>& tvecs,
                                                std::vector<std::vector<cv::Point2f> >& imagePoints,
                                                std::vector<std::vector<cv::Point3f> >& scenePoints) const {
...
void CameraCalibrationTest::optimize(CameraPtr& camera, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
                                     const std::vector<std::vector<cv::Point2f> >& imagePoints,
                                     const std::vector<std::vector<cv::Point3f> >& scenePoints) const {
  ...
  for (size_t i = 0; i < imagePoints.size(); ++i) {
    for (size_t j = 0; j < imagePoints.at(i).size(); ++j) {
      ...
      problem.AddResidualBlock(costFunction, lossFunction, transformVec.at(i).rotationData(),
...
template <typename T>
void CameraCalibrationTest::readData(std::ifstream& ifs, T& data) const {
...
template <typename T>
void CameraCalibrationTest::writeData(std::ofstream& ofs, T data) const {
