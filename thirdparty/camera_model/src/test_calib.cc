...
#pragma omp parallel for private(image_index)
  ...
  if (viewResults && verbose) {
    std::cout << "\033[32;40;1m"
              << "# INFO: Used image num: " << calibTest.m_ImagesShow.size() << "\033[0m" << std::endl;
    ...
    for (size_t i = 0; i < calibTest.m_ImagesShow.size(); ++i) {
      ...
      if (is_save_images) {
        ...
        ss << i;
        cv::imwrite(result_images_save_folder + "calib_result_" + ss.str() + ".jpg", calibTest.m_ImagesGoodShow.at(i));
      }
      cv::waitKey(0);
    }
  }
