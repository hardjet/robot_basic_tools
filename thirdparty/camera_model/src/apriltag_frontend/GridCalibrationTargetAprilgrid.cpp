...
GridCalibrationTargetAprilgrid::GridCalibrationTargetAprilgrid(size_t tagRows, size_t tagCols, double tagSize,
  ...
  if (tagSize < 0.0) std::cout << "tagSize has to be positive \n";
  if (tagSpacing < 0.0) std::cout << "tagSpacing has to be positive \n";
...
bool GridCalibrationTargetAprilgrid::computeObservation(const cv::Mat& src_image, cv::Mat& dst_image,
  ...
  for (iter = detections.begin(); iter != detections.end();) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      remove |= iter->p[j].first > (float)(src_image.cols) - _options.minBorderDistance;  // width
      remove |= iter->p[j].second < _options.minBorderDistance;
      remove |= iter->p[j].second > (float)(src_image.rows) - _options.minBorderDistance;  // height
    }
  ...
  if (detections.size() > 1) {
    for (unsigned i = 0; i < detections.size() - 1; i++)
      if (detections[i].id == detections[i + 1].id) {
        ...
        cv::Mat imageCopy = src_image.clone();
  ...
  if (_options.doSubpixRefinement && success)
    cv::cornerSubPix(src_image, tagCorners, cv::Size(2, 2), cv::Size(-1, -1),
  ...
  for (unsigned i = 0; i < detections.size(); i++)
    for (unsigned j = 0; j < 4; j++) {
      ...
      // cv::circle(imageCopy1, cv::Point2f(detections[i].p[j].first, detections[i].p[j].second), 2, CV_RGB(255,0,0),
      // 1);

      // subpixel refined corners
      cv::circle(dst_image, cv::Point2f(tagCorners.at<float>(4 * i + j, 0), tagCorners.at<float>(4 * i + j, 1)), 3,
                 CV_RGB(0, 0, 255), 1);

      if (!success)
        cv::putText(dst_image, "Detection failed! (frame not used)", cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 0.8,
                    CV_RGB(255, 0, 0), 3, 8, false);
    }

  /* highlight detected tags in image */
  for (unsigned i = 0; i < detections.size(); i++) {
    detections[i].draw(dst_image);
  }

  // cv::imshow("Aprilgrid Board", dst_image);  // OpenCV call
  // cv::waitKey(1);
...
bool GridCalibrationTargetAprilgrid::computeObservation(const cv::Mat& image, std::vector<cv::Point2f>& points2ds,
