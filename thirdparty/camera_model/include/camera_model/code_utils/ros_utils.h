...
inline void sendDepthImage(ros::Publisher& pub, const ros::Time timeStamp, const std::string frame_id,
                           const cv::Mat& depth) {
...
inline void sendCloud(ros::Publisher& pub, const ros::Time timestamp, const cv::Mat& dense_points_,
                      const cv::Mat& un_img_l0, Eigen::Matrix3f K1, Eigen::Matrix3f R_wc) {
#define DOWNSAMPLE 0
  ...
  for (int32_t u = 0; u < dense_points_.rows; ++u) {
    for (int32_t v = 0; v < dense_points_.cols; ++v, ++i) {
      ...
      if (dep > 5) continue;
      if (dep < 0.95) continue;
      if (Point_w(2) > 0.5) continue;
...
inline void pointCloudPub(ros::Publisher& pub, cv::Mat depth, cv::Mat image) {
  int w, h, level;
  level = 0;
  w = (int)(depth.cols);
  h = (int)(depth.rows);
