...
#include "algorithm/random/random_shuffle.h"
...
class CRandomGenerator {
 ...
 public:
  ...
  explicit CRandomGenerator(const uint32_t seed) { randomize(seed); }
  ...
  return_t drawUniform(double Min, double Max) {
   ...
   */
  template <typename return_t = double>
  return_t drawGaussian1D(double mean, double std) {
  ...
  MATRIX drawDefinitePositiveMatrix(const size_t dim, const double std_scale = 1.0,
  ...
  void drawGaussianMultivariate(std::vector<T>& out_result, const MATRIX& cov, const std::vector<T>* mean = nullptr) {
    ...
    if (cov.rows() != cov.cols()) throw std::runtime_error("drawGaussianMultivariate(): cov is not square.");
    if (mean && mean->size() != dim) throw std::runtime_error("drawGaussianMultivariate(): mean and cov sizes ");
    MATRIX Z, D;
  ...
  void drawGaussianMultivariate(VECTORLIKE& out_result, const COVMATRIX& cov, const VECTORLIKE* mean = nullptr) {
    ...
    if (cov.rows() != cov.cols()) throw std::runtime_error("drawGaussianMultivariate(): cov is not square.");
    if (mean && size_t(mean->size()) != N) throw std::runtime_error("drawGaussianMultivariate(): mean and cov sizes ");
    ...
    for (typename COVMATRIX::Index c = 0; c < eigVecs.cols(); c++) {
      ...
      for (typename COVMATRIX::Index r = 0; r < eigVecs.rows(); r++) eigVecs(c, r) *= s;
    }
  ...
  void drawGaussianMultivariateMany(VECTOR_OF_VECTORS& ret, size_t desiredSamples, const COVMATRIX& cov,
                                    const typename VECTOR_OF_VECTORS::value_type* mean = nullptr) {
    const size_t N = cov.rows();
    if (cov.rows() != cov.cols()) throw std::runtime_error("drawGaussianMultivariateMany(): cov is not square.");
  ...
  void permuteVector(const VEC& in_vector, VEC& out_result) {
    ...
    if (N > 1) algorithm::random::shuffle(&out_result[0], &out_result[N - 1]);
  }
 ...
 */
inline ptrdiff_t random_generator_for_STL(ptrdiff_t i) { return getRandomGenerator().drawUniform32bit() % i; }
...
void matrixRandomUni(MAT& matrix, const double unif_min = 0, const double unif_max = 1) {
  for (typename MAT::Index r = 0; r < matrix.rows(); r++)
    for (typename MAT::Index c = 0; c < matrix.cols(); c++)
      matrix(r, c) = static_cast<typename MAT::Scalar>(getRandomGenerator().drawUniform(unif_min, unif_max));
}
...
void matrixRandomNormal(MAT& matrix, const double mean = 0, const double std = 1) {
  for (typename MAT::Index r = 0; r < matrix.rows(); r++)
    for (typename MAT::Index c = 0; c < matrix.cols(); c++)
      matrix(r, c) = static_cast<typename MAT::Scalar>(mean + std * getRandomGenerator().drawGaussian1D_normalized());
}
...
void vectorRandomNormal(std::vector<T>& v_out, const T& mean = 0, const T& std = 1) {
  size_t n = v_out.size();
  for (size_t r = 0; r < n; r++) v_out[r] = mean + std * getRandomGenerator().drawGaussian1D_normalized();
}
 ...
 */
template <typename T, typename MATRIX>
void randomNormalMultiDimensionalMany(const MATRIX& cov, size_t desiredSamples, std::vector<std::vector<T>>& ret,
                                      std::vector<T>* samplesLikelihoods = nullptr) {
  getRandomGenerator().drawGaussianMultivariateMany(ret, desiredSamples, cov,
                                                    static_cast<const std::vector<T>*>(nullptr), samplesLikelihoods);
}
