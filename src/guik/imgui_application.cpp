...
bool Application::init(const char *window_name, const char *imgui_config_path, const Eigen::Vector2i &size, const char *glsl_version) {
  glfwSetErrorCallback([](int err, const char *desc) { std::cerr << "glfw error " << err << ": " << desc << std::endl; });        // 匿名函数 + 函数类型
  if (!glfwInit()) {                                              // 初始化GLFW
    std::cerr << "failed to initialize GLFW" << std::endl;        // 初始化失败执行的操作
    return false;
  }

  glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);                  // 为下一次glfwCreateWindow函数的调用设置hints，对上下文环境进行相应的配置，调用通常在glfwCreateWindow前，而且这些hints设置以后将会保持不变
  glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);                  // 设置OpenGL的最低主版本，设置OpenGL的最低副版本

  window = glfwCreateWindow(size[0], size[1], window_name, nullptr, nullptr);       // GLFWwindow* widnow; 在构造函数中被初始化为nullptr，参数=窗口高度，宽度，窗口标题，窗口显示模式（null=窗口，不全屏），是否和窗口共享资源
  ...
  glfwSetFramebufferSizeCallback(window, fb_size_callback);       // 注册函数，注册回调函数为fb_size_callback()，实际调用了framebuffer_size_callback()，返回窗口的尺寸

  glfwMakeContextCurrent(window);                                 // 设置当前窗口的上下文环境，设置完之后，所有的OpenGL命令都会用来改变当前的上下文的状态信息
