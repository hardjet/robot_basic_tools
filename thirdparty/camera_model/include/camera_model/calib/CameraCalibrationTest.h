...
class CameraCalibrationTest {
 public:
  ...
  double reprojectionError(const std::vector<std::vector<cv::Point3f> >& objectPoints,
                           ...
                           const std::vector<cv::Mat>& tvecs, cv::OutputArray _perViewErrors = cv::noArray()) const;
  ...
  void drawResultsFiltered(std::vector<cv::Mat>& images, std::vector<std::string>& imageNames,
                           cv::Mat& DistributedImage) const;
  void drawResults(std::vector<cv::Mat>& images, std::vector<std::string>& imageNames, const cv::Mat& DistributedImage,
                   const std::vector<cv::Mat> cameraPoseRs, const std::vector<cv::Mat> cameraPoseTs,
 ...
 private:
  bool calibrateHelper(CameraPtr& camera, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
                       std::vector<std::vector<cv::Point2f> >& imagePoints,
                       std::vector<std::vector<cv::Point3f> >& scenePoints) const;
  bool CalibrationDataTest(CameraPtr& camera, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
                           std::vector<std::vector<cv::Point2f> >& imagePoints,
                           std::vector<std::vector<cv::Point3f> >& scenePoints) const;

  void optimize(CameraPtr& camera, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
                const std::vector<std::vector<cv::Point2f> >& imagePoints,
                const std::vector<std::vector<cv::Point3f> >& scenePoints) const;
