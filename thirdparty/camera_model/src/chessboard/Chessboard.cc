...
bool Chessboard::findChessboardCornersImproved(const cv::Mat& image, const cv::Size& patternSize,
  ...
  if (image.depth() != CV_8U || image.channels() == 2) return false;
  if (patternSize.width < 2 || patternSize.height < 2) return false;
  if (patternSize.width > 127 || patternSize.height > 127) return false;
  if (patternSize.width == patternSize.height) return false;
