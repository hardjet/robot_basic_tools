...
void ArcCameraControl::drag(const Eigen::Vector2i& p, int button) {
  ...
  if (middle_button_down) {
    center += Eigen::AngleAxisf(theta + M_PI_2, Eigen::Vector3f::UnitZ()) * Eigen::Vector3f(-rel[0], rel[1], 0.0f) *
              distance * 0.001f;
  }
...
double ArcCameraControl::access_distance() const { return distance; }
...
Eigen::Matrix4f ArcCameraControl::view_matrix() const {
  ...
  glm::mat4 mat = glm::lookAt(glm::vec3(eye[0], eye[1], eye[2]), glm::vec3(center[0], center[1], center[2]),
